...
from spacy_conllu.utils import PD_AVAILABLE, merge_dicts_strict
...
class ConlluFormatter:
    ...
    - `conll_str`: string representation of the CoNLL format
        ...
        - in sentence `Span`: the expected CoNLL format where each row represents a token. When
          `ConlluFormatter(include_headers=True)` is used, two header lines are included as well, as per the
          `CoNLL format`_.
    ...
    def _set_extensions(self):
        ...
        # However, ConlluParser might set these fields when it has read CoNLL_str->spaCy

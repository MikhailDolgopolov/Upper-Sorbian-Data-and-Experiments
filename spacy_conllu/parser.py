...
from spacy_conllu.utils import STANZA_AVAILABLE, UDPIPE_AVAILABLE
...
class ConlluParser:
    """Constructor for a ConlluParser, which is a wrapper around a spaCy-like parser with a ConlluFormatter
    ...
    def __post_init__(self):
        if "conllu_formatter" not in self.nlp.pipe_names:
            raise ValueError(Errors.E001.format(name="conllu_formatter", opts=self.nlp.pipe_names))
    ...
    def parse_text_as_conll(
        ...
        if n_process > 1 and not ignore_pipe_errors:
            if not self.nlp.get_pipe("conllu_formatter").disable_pandas:
                raise OSError(
                    "Due to pandas serialisation, 'n_process' > 1 is not supported when"
                    " 'disable_pandas' is False in the ConlluFormatter. Set 'n_process' to 1 or"
                    " initialise the ConlluFormatter with 'disable_pandas=True'"
                )
        ...
        for doc_idx, doc in enumerate(self.nlp.pipe(text, n_process=n_process)):
            for sent in doc.sents:
                ...
                if self.nlp.get_pipe("conllu_formatter").include_headers and not no_force_counting:
                    # nlp.pipe returns different docs, meaning that the generated sentence indices
                    # by ConlluFormatter are not consecutive (they reset for each new doc)
    ...
    def parse_conllu_text_as_spacy(
        ...
        for chunk in text.split("\n\n"):
            ...
            for i in range(len(lines)):
                ...
                if "." in id_ or "-" in id_:
                    raise NotImplementedError("Multi-word tokens and empty nodes are not supported in spacy_conllu")
